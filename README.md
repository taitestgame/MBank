...
---
## 💻 Ví dụ sử dụng "Mỳ ăn liền"

```python
from mb import MB

# Khởi tạo robot
bot = MB(username="TEN_DANG_NHAP", password="MAT_KHAU")

# Đăng nhập (Auto giải Captcha)
if bot.login().get("result", {}).get("ok"):
    print("✅ Đăng nhập MB thành công!")
    
    # In số dư đẹp mắt
    for acc in bot.get_balance():
        print(f"💳 {acc['number']} | {acc['balance']:,.0f} {acc['currency']}")
else:
    print("❌ Thất bại. Kiểm tra lại thông tin hoặc Captcha!")